...
namespace Demo.Scripts.Runtime.Layers
{
    public class DemoLoco : LocomotionLayer
    {
        ...
        protected virtual void ApplyLocomotion()
        {
            ...
            CoreToolkitLib.RotateInBoneSpace(GetRootBone().rotation, GetPelvis(), sprintLean);
            ...
            CoreToolkitLib.MoveInBoneSpace(GetRootBone(), master,
                Vector3.Lerp(Vector3.zero, curveData / 100f, smoothLocoAlpha));

            CoreToolkitLib.RotateInBoneSpace(GetRootBone().rotation, master,
                Quaternion.Slerp(Quaternion.identity, animRot, smoothLocoAlpha));

            CoreToolkitLib.MoveInBoneSpace(GetRootBone(), master,
                Vector3.Lerp(Vector3.zero, sprintPose.position, sprintAlpha));
